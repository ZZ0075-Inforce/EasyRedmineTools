...
const APP_HTML = `<div class="pj-app-shell">
    ...
    <div class="shell">
      ...
      <div class="pj-main-view" data-view="schedule" hidden>
      <section class="layout">
        <article class="panel pj-list-panel">
          <div class="pj-workbench-header">
            <div>
              <span class="pj-section-title">兩週排程</span>
              <div class="muted" id="schedule-list-summary">選擇要排程的專案</div>
            </div>
          </div>
          <ul class="pj-issue-list" id="schedule-issue-list"></ul>
        </article>
        <aside class="panel pj-details-panel">
          <div class="pj-workbench-header">
            <div>
              <span class="pj-section-title">甘特分配</span>
              <div class="muted">每筆 issue 預估時數會自動分配到工作日</div>
            </div>
          </div>
          <div class="pj-alert-stack" id="schedule-alert-stack"></div>
          <div class="pj-table-wrap" id="schedule-table-wrap"></div>
      ...
      <div class="pj-modal-box pj-settings-modal-box" role="dialog" aria-modal="true" aria-labelledby="settings-modal-title">
        ...
        <div class="pj-settings-modal-content">
          <div class="pj-settings-tabs" role="tablist" aria-orientation="vertical">
            ...
            <button class="pj-settings-tab" data-settings-tab="ai" role="tab">AI 助手</button>
    ...
    <div id="ai-suggest-modal" class="pj-modal-overlay" hidden aria-hidden="true">
      <div class="pj-modal-box pj-modal-box-md" role="dialog" aria-modal="true" aria-labelledby="ai-suggest-modal-title">
        ...
        <div class="pj-modal-body">
          ...
          <div class="pj-ai-tab-bar" role="tablist">
            <button type="button" class="pj-ai-tab active" data-ai-tab="new" role="tab">✨ 新生成</button>
            <button type="button" class="pj-ai-tab" data-ai-tab="history" role="tab">🕘 歷史紀錄</button>
          </div>
          <div class="pj-ai-tab-panel" data-ai-tab-panel="new">
            <label class="pj-field-stack">
          ...
          <div class="pj-ai-tab-panel" data-ai-tab-panel="history" hidden>
            <div class="muted">最近 7 天的 AI 建議紀錄（最多 100 筆）。點任一筆載入，可重新勾選加入。</div>
            <div id="pj-ai-history-list" class="pj-ai-history-list" style="margin-top:12px;"></div>
            <button type="button" class="pj-ghost-button" id="ai-history-clear" style="margin-top:12px;">清空全部</button>
          </div>
        </div>
        <div class="pj-modal-footer">
          <button class="btn btn-secondary" id="ai-suggest-modal-cancel">取消</button>
          <button class="btn btn-primary" id="ai-suggest-apply" disabled>加入勾選項</button>
        </div>
      </div>
    </div>
...
const APP_CSS = `#__worklog_root {
      ...
      @keyframes phrase-menu-in {
        ...
        gap: 8px;
        pointer-events: none;
      }#__worklog_root .pj-ai-tab-bar {
        display: flex; gap: 4px;
        margin: 12px 0;
        border-bottom: 1px solid var(--panel-border);
      }#__worklog_root .pj-ai-tab {
        background: none; border: 0;
        padding: 8px 12px; cursor: pointer;
        color: var(--muted); font-size: 13px;
        border-bottom: 2px solid transparent;
        margin-bottom: -1px;
      }#__worklog_root .pj-ai-tab:hover { color: var(--ink); }#__worklog_root .pj-ai-tab.active {
        color: var(--ink); font-weight: 600;
        border-bottom-color: var(--accent);
      }#__worklog_root .pj-ai-tab-panel[hidden] { display: none; }#__worklog_root .pj-ai-history-list { display: flex; flex-direction: column; gap: 8px; }#__worklog_root .pj-ai-history-item {
        border: 1px solid var(--panel-border);
        border-radius: 8px;
        padding: 10px 12px;
        cursor: pointer;
        background: var(--panel);
      }#__worklog_root .pj-ai-history-item:hover { background: var(--subtle); border-color: var(--accent); }#__worklog_root .pj-ai-history-item-meta {
        font-size: 12px; color: var(--muted);
        margin-bottom: 4px;
      }#__worklog_root .pj-ai-history-item-task {
        font-weight: 500;
        font-size: 13px;
        color: var(--ink);
      }#__worklog_root .pj-ai-history-item-context {
        font-size: 12px; color: var(--muted);
        margin-top: 4px;
        overflow: hidden; text-overflow: ellipsis;
        white-space: nowrap;
      }#__worklog_root .pj-ai-history-item-suggest-count {
        font-size: 12px; color: var(--muted);
        margin-top: 4px;
      }#__worklog_root .pj-ai-history-empty {
        text-align: center;
        color: var(--muted);
        padding: 24px;
      ...
      <details class="settings-ai-section" style="margin-top: 24px;">
        ...
        <div class="pj-field-stack" style="margin-top: 12px;">
          ...
          <div class="ai-settings-actions" style="display:flex; gap:8px; align-items:center; margin-top: 12px;">
            <button type="button" class="pj-action-button" data-ai-agent-save="${agentId}">儲存</button>
            <button type="button" class="pj-ghost-button" data-ai-agent-reset="${agentId}">重設為預設</button>
...
const PhraseMenu = (() => {
  ...
  // Phrase popup 用 position: fixed 跳出 .pj-table-wrap overflow clip
  ...
  // capture: true 抓 nested scroll container（pj-main-view / panel / pj-table-wrap）
...
// schedule view 已獨立成 pj-main-view (data-view="schedule")，render 寫進
  ...
  function bindScheduleDrag() {
    for (const el of elements.scheduleIssueList.querySelectorAll("[data-project-drag-id]")) {
      ...
      el.addEventListener("dragend", () => {
        dragContext = null;
        for (const n of elements.scheduleIssueList.querySelectorAll(".pj-drag-over")) n.classList.remove("pj-drag-over");
      });
    }
...
const AISuggestModal = (() => {
  ...
  function renderHistory() {
    ...
    if (!entries.length) {
      elements.aiHistoryList.innerHTML = `<div class="pj-ai-history-empty">尚無歷史紀錄。生成一次後會自動記下。</div>`;
      return;
    }
    elements.aiHistoryList.innerHTML = entries.map((e) => {
      ...
      return `
        <div class="pj-ai-history-item" data-ai-history-id="${escapeHtml(e.id)}">
          <div class="pj-ai-history-item-meta">${escapeHtml(tsStr)} · ${escapeHtml(e.role || "?")}</div>
          <div class="pj-ai-history-item-task">${escapeHtml(taskPreview)}</div>
          ${ctxPreview ? `<div class="pj-ai-history-item-context">背景：${escapeHtml(ctxPreview)}</div>` : ""}
          <div class="pj-ai-history-item-suggest-count">${count} 筆建議</div>
        </div>
      `;
    }).join("");
    ...
    for (const btn of elements.savedQueriesList.querySelectorAll("[data-remove-query]")) {
      btn.addEventListener("click", async (event) => {
        ...
        try {
          ...
          renderAll();  // pj-source-tabs + saved-queries-list + 可能切到 mine 後的 issue 列表
        } catch (error) {
...
const elements = {
  ...
  aiTabBar: document.querySelector("#ai-suggest-modal .pj-ai-tab-bar"),
  ...
  aiHistoryList: document.getElementById("pj-ai-history-list"),
...
function renderAlerts() {
  // 主 pj-alert-stack（worklog view 內）：issue/activity/preview 相關 + 送出結果
  const main = [];
